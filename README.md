...
\[
R_{t,i}
= \text{eff}_i \cdot \frac{A_{t,i}^n}{k^n + A_{t,i}^n}
\]
...
\[
a_{t+1,i} = a_{t,i} + \varepsilon^a_{t,i},\quad
b_{t+1,i} = b_{t,i} + \varepsilon^b_{t,i}
\]
...
\[
\text{eff}_{t,i}^\star
= \text{clip}\left(\text{eff}_i \cdot f(a_{t,i}, b_{t,i})\right)
\]
...
\text{Reward}_t
=
\underbrace{\sum_i R_{t,i}}_{\text{Channel ROI}}
+
\underbrace{s_t^\top C s_t}_{\text{Synergy}}
+
\underbrace{\text{pacing\_pen}_t}_{\text{Budget Pacing}}
-
\underbrace{\text{smooth\_pen}\,\|\pi_t-\pi_{t-1}\|_2}_{\text{Stability}}
+
